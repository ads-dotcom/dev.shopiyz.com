...
const renderOpenApi = (catalog) => {
  ...
  lines.push(
    ...
    "      type: object",
    "      additionalProperties: false",
    "      required: [provider, providerKey, enabled, label, checkoutPath, acceleratedPaymentProviders]",
    "      properties:",
    "        provider: { type: string, enum: [external] }",
    "        providerKey: { type: string, enum: [shopify-checkout, payment-paytr, payment-iyzico] }",
    "        enabled: { type: boolean }",
    "        label: { type: string }",
    "        checkoutPath:",
    "          type: string",
    "          enum: [/api/storefront/v1/shopify-checkout, /api/storefront/v1/payment-checkout]",
    "        requiresIdentityNumber: { type: boolean }",
    "        requiresBillingAddress: { type: boolean }",
    "        acceleratedPaymentProviders:",
    "          type: array",
    "          items: { type: string, enum: [shop_pay] }",
    "    ShopResponse:",
    "      allOf:",
    "        - $ref: '#/components/schemas/StorefrontMeta'",
    "        - type: object",
    "          required: [shop, storefront, branding, seo, settings, localization, promotions, checkout]",
    "          properties:",
    ...
    "            checkout:",
    "              oneOf:",
    "                - $ref: '#/components/schemas/StorefrontCheckoutConfig'",
    "                - type: 'null'",
